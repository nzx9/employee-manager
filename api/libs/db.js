...
class Employee {
  static async create({
    ...
    const res = await exec(
      ...
      [first_name, last_name, email, gender, phone, photo]
    );
    if (res.rowCount > 0) {
      return res.rows[0];
    } else {
  ...
  static async findById(empId) {
    const res = await exec("SELECT * FROM employees WHERE empId = $1", [empId]);
    if (res.rowCount > 0) return res.rows[0];
    else return null;
  }

  static async findAll() {
    const res = await exec("SELECT * FROM employees ORDER BY empId");
    return res.rows;
  }

  static async deleteById(empId) {
    return await exec("DELETE FROM employees WHERE empId = $1", [empId]);
  }
}
